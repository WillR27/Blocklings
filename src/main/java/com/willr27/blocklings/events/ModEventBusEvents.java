...
public class ModEventBusEvents
{
    ...
    {
        final NativeImage maskNativeImage = SimpleTexture.TextureData.load(Minecraft.getInstance().getResourceManager(), new BlocklingsResourceLocation("textures/entity/blockling/mask.png")).getImage();
        ...
        for (BlocklingType baseBlocklingType : BlocklingType.TYPES)
        {
            for (BlocklingType outerBlocklingType : BlocklingType.TYPES)
            {
                ...
                Minecraft.getInstance().textureManager.register(baseBlocklingType.getCombinedTexture(outerBlocklingType), new DynamicTexture(baseNativeImage));
            }
        }
    }
}
