...
import com.willr27.blocklings.client.gui2.controls.Anchor;
import com.willr27.blocklings.client.gui2.controls.common.ScrollbarControl;
import com.willr27.blocklings.client.gui2.controls.common.panel.PanelControl;
...
public class Control extends AbstractGui implements IControl
{
    ...
    /**
     * Whether the control's children can be dragged outside it.
     */
    private boolean canBeDraggedOutOf = false;
    ...
    /**
     * Tries to update the control's position if it is being dragged.
     *
     * @param mouseX the scaled mouse x position.
     * @param mouseY the scaled mouse y position.
     */
    protected void tryUpdateDraggedPosition(int mouseX, int mouseY)
    {
        if (isDragging() && getParent() instanceof Control)
        {
            Control parent = (Control) getParent();

            setX((int) (parent.toLocalX(mouseX) / getEffectiveScale()) - getEffectiveWidth() / 2);
            setY((int) (parent.toLocalY(mouseY) / getEffectiveScale()) - getEffectiveHeight() / 2); // TODO: The mouse is NOT scaled!!!
        }
    }
    ...
    {
        tryUpdateDraggedPosition(mouseX, mouseY);
    ...
    /**
     * Fills the control boundaries with the given colour.
     *
     * @param matrixStack the matrix stack.
     * @param colour the colour to fill the control with.
     */
    public void fill(@Nonnull MatrixStack matrixStack, int colour)
    {
        fill(matrixStack, getScreenX(), getScreenY(), getScreenX() + getEffectiveWidth(), getScreenY() + getEffectiveHeight(), colour);
    }
...
//        font.draw(matrixStack, shadowText, screenX + bonusX + 1, screenY + dy + 1, 0x222222);
    ...
    {
        // Don't apply anchoring if the parent is a panel.
        if (parent instanceof PanelControl)
        {
            return;
        }
    ...
    @Override
    public int setScrollX(int scroll)
    {
        int beforeScrollX = scrollX;
    ...
    @Override
    public int setScrollY(int scroll)
    {
        int beforeScrollY = scrollY;
    ...
    @Override
    public boolean canBeDraggedOutOf()
    {
        return canBeDraggedOutOf;
    }

    @Override
    public void setCanBeDraggedOutOf(boolean canBeDraggedOutOf)
    {
        this.canBeDraggedOutOf = canBeDraggedOutOf;
    }
