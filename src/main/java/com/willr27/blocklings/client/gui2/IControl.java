...
public interface IControl
{
     ...
     */
    default void TODOrenamePreRenderAll(int mouseX, int mouseY, float partialTicks)
    {
        if (isVisible())
        {
            TODOrenamePreRender(mouseX, mouseY, partialTicks);

            getChildrenCopy().forEach(control -> control.TODOrenamePreRenderAll(mouseX, mouseY, partialTicks));
        }
    }
     ...
     * @param partialTicks the fraction of ticks that have occurred since the last frame.
     */
    default void TODOrenamePreRender(int mouseX, int mouseY, float partialTicks)
    {
    ...
    default void renderAll(@Nonnull MatrixStack matrixStack, int mouseX, int mouseY, float partialTicks)
    {
        ...
        else
        {
            ...
            if (isVisible())
            {
                ...
                postRender(matrixStack, mouseX, mouseY, partialTicks);
    ...
    /**
     * Called directly before {@link #render}. Useful for applying transformations before rendering
     * the control.
     *
     * @param matrixStack the current matrix stack.
     * @param mouseX the scaled mouse x position.
     ...
     * @param partialTicks the fraction of ticks that have occurred since the last frame.
     */
    default void preRender(@Nonnull MatrixStack matrixStack, int mouseX, int mouseY, float partialTicks)
    {
        if (isDragging())
        {
    ...
    /**
     * Called directly after {@link #render}. Useful for removing transformations before rendering
     * the control.
     *
     * @param matrixStack the current matrix stack.
     * @param mouseX the scaled mouse x position.
     ...
     * @param partialTicks the fraction of ticks that have occurred since the last frame.
     */
    default void postRender(@Nonnull MatrixStack matrixStack, int mouseX, int mouseY, float partialTicks)
    {
        if (isDragging())
        {
            matrixStack.translate(0.0f, 0.0f, -500.0f);
    ...
    default void controlMouseScrolled(@Nonnull MouseScrollEvent e)
    {
        ...
        {
            e.setIsHandled(setScrollX(getScrollX() - adjustedScroll) != 0);
        }
        else
        {
            e.setIsHandled(setScrollY(getScrollY() - adjustedScroll) != 0);
        }
     ...
     */
    default int setScrollX(int scroll)
    {
        return 0;
    }
     ...
     * @return the amount actually scrolled.
     */
    default int setScrollY(int scroll)
    {
        return 0;
    }
    ...
    /**
     * @return whether the control's children can be dragged outside it.
     */
    default boolean canBeDraggedOutOf()
    {
        return false;
    }

    /**
     * Sets whether the control's children can be dragged outside it.
     */
    default void setCanBeDraggedOutOf(boolean canBeDraggedOutOf)
    {
