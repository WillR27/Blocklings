...
public abstract class PathNavigateBlockling extends PathNavigate
{
    ...
    public PathNavigateBlockling(EntityLiving entityIn, World worldIn)
    {
    ...
    public Path getPathToPosWithRemovedBlock(BlockPos pos, BlockPos removedPos)
    {
        if (!this.canNavigate())
        {
            return null;
        ...
        else
        {
            ...
            BlockPos blockpos = new BlockPos(this.entity);
            int i = (int)(f + 8.0F);
            ChunkCacheBlockling chunkcache = new ChunkCacheBlockling(this.world, blockpos.add(-i, -i, -i), blockpos.add(i, i, i), removedPos, 0);
            Path path = this.pathFinder.findPath(chunkcache, this.entity, this.targetPos, f * 2);
            this.world.profiler.endSection();
            return path;
        }
    }
    ...
    /**
     * Checks if entity haven't been moved when last checked and if so, clears current path
     **/
